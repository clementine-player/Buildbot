...
def MakeDebBuilder(distro, version, is_64_bit):
  ...
  f.addStep(
      shell.Compile(
          command=make_cmd, haltOnFailure=True, workdir="source/bin", env=env))
...
def MakePPABuilder(distro, ppa):
  ...
  f.addStep(
      shell.ShellCommand(
          name="clean", command=clean_cmd, haltOnFailure=True, workdir="."))
  f.addStep(
      shell.ShellCommand(
          name="maketarball",
  ...
  f.addStep(
      shell.ShellCommand(
          name="dput", command=dput_cmd, haltOnFailure=True, workdir="."))
  return f
...
def MakeWindowsDepsBuilder():
  ...
  f.addStep(
      shell.ShellCommand(
          name="clean", workdir="source/windows", command=["make", "clean"]))
  f.addStep(
      shell.ShellCommand(
          name="compile", workdir="source/windows", command=["make"]))
  return f
...
def MakeSpotifyBlobBuilder():
  ...
  f.addStep(
      shell.SetProperty(
          command=["echo", SPOTIFYBASE], property="spotifybase"))
  f.addStep(
...
def MakeMacBuilder():
  ...
  f.addStep(
      shell.Compile(
          command=["make"], workdir="source/bin", haltOnFailure=True))
  f.addStep(
      shell.ShellCommand(
          name="install",
          command=["make", "install"],
...
def MakeMacDepsBuilder():
  ...
  f.addStep(
      shell.ShellCommand(
          name="clean", workdir="/src/macosx", command=["make", "clean"]))
  f.addStep(
      shell.ShellCommand(
          name="compile", workdir="/src/macosx", command=["make"]))
  return f


def _AddTxSetup(f, resource, source_file, pattern, pot=True):
  set_args = [
      "tx", "set", "--auto-local", "-r", resource, pattern, "--source-lang",
      "en", "--execute"
  ]
  ...
  f.addStep(
      shell.ShellCommand(
          name="clean", workdir="source", command=["rm", "-rf", ".tx"]))
  f.addStep(
      shell.ShellCommand(
          name="tx_init",
          workdir="source",
  ...
  f.addStep(
      shell.ShellCommand(
          name="tx_set", workdir="source", haltOnFailure=True,
          command=set_args))
...
def _AddGithubSetup(f):
  ...
  f.addStep(
      shell.ShellCommand(
          ...
          haltOnFailure=True,
          command=[
              "git", "config", "--global", "user.name", "Clementine Buildbot"
          ]))
...
def _MakeTransifexPoPullBuilder(repo, po_glob):
  ...
  f.addStep(
      shell.ShellCommand(
          ...
          haltOnFailure=True,
          command=[
              "git", "push", "git@github-%s:clementine-player/%s.git" % (
                  repo, repo), "master", "--verbose"
          ]))
  return f
...
def MakeWebsiteTransifexPoPullBuilder():
  return _MakeTransifexPoPullBuilder("Website",
                                     "www.clementine-player.org/locale/*.po")


def MakeAndroidTransifexPoPullBuilder():
  return _MakeTransifexPoPullBuilder("Android-Remote",
...
def MakeTransifexPotPushBuilder():
  ...
  f.addStep(
      shell.Compile(
          haltOnFailure=True, workdir="source/bin", command=["make", "-j4"]))
...
def MakeAndroidRemoteBuilder():
  ...
  f.addStep(
      shell.ShellCommand(
          name="sed", command=sed_cmd, haltOnFailure=True, workdir="source"))
  f.addStep(
      shell.ShellCommand(
          name="compile",
          haltOnFailure=True,
